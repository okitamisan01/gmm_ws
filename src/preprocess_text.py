...
def _wiki_langlink(title: str, from_lang: str = "en", to_lang: str = "ja") -> str:
    """Resolve a title from one language to another using langlinks. Return destination title or ''."""
    if requests is None:
        raise RuntimeError("requests が見つかりません。requirements.txt に requests を追加してください。")
...
def _generate_query_variants(name: str) -> List[str]:
    ...
    variants = []
    # ① override があれば最優先
    if name in JA_TITLE_OVERRIDES:
        variants.append(JA_TITLE_OVERRIDES[name])
    if base in JA_TITLE_OVERRIDES:
        variants.append(JA_TITLE_OVERRIDES[base])
    # ② 通常化した候補
    variants += [name, base]
    ...
    for k,v in HINTS.items():
        if k in key:
            variants.append(f"{JA_TITLE_OVERRIDES.get(base, base)} {v}")
            break
    # 重複除去
    seen, uniq = set(), []
    for v in variants:
